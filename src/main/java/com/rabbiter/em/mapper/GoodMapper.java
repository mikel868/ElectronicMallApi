...
public interface GoodMapper extends BaseMapper<Good> {
    ...
    @Select("SELECT discount * price FROM good_standard gs, good WHERE good.id = gs.good_id AND good.id = #{id} ORDER BY gs.price ASC LIMIT 1")
    BigDecimal getCurrentPrice(Long id);
    ...
    @Select("SELECT gs.price FROM good_standard gs " +
            "INNER JOIN good g ON gs.good_id = g.id " +
            "WHERE gs.`value` = #{name} AND gs.good_id = #{id} AND g.is_delete = 0")
    BigDecimal getStandardPriceByNameAndId(@Param("name") String name, @Param("id") Long id);
